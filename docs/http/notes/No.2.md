...
5. 浏览器通过网络接收数据，解析 `HTML` 并构造 `DOM树`
6. CSS解析器处理CSS样式并构建 `CSSOM树`（CSS对象模型树）
7. 如果存在 `JavaScript`，则执行 `JavaScript`，并修改 `DOM树` 和 `CSSOM树`
8. 将解析创建的 `DOM树` 和 `CSSOM树` 合成一个渲染树（ Rendering Tree）
9. 根据渲染树计算每个可见元素的布局，然后将其绘制到屏幕上（在某些情况下，可以将内容提升到它们自己的层并进行合成，通过在 `GPU` 而不是 `CPU` 上绘制屏幕的一部分来提高性能，从而释放主线程。）
  布局是确定渲染树中所有节点的尺寸和位置，以及确定页面上每个对象的大小和位置的过程
10. 分层：主线程使用一套复杂的策略对整个布局树进行分层。浏览器中的 `Layers` 可以查看每个页面的分层效果。
11. 绘制：在每个层进行绘制过程，主线程会为每个层单独产生绘制指令集，用于描述这一层的内容该如何画出来。完成绘制后，主线程将每个图层的绘制信息提交给合成线程，剩余工作将由合成线程完成。

    合成线程会对每个图层进行分块，将其划分为更多的小区域；分块完成后，进入光栅化阶段，合成线程会将块信息交给 GPU 进程，以极高的速度完成光栅化。GPU 进程会开启多个线程来完成光栅化，并且优先处理靠近视口区域的块。

12. 合成：在每个层上完成绘制过程之后，浏览器会将所有层按照合理的顺序合并成一个图层，然后显示在屏幕上。对于有位置重叠的元素的页面，这个过程尤其重要，因为一旦图层的合并顺序出错，将会导致元素显示异常。
13. 显示在用户浏览器页面
14. 最后，客户端与服务器进行TCP四次挥手断开连接，也可能复用连接
