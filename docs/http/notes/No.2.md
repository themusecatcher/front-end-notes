...
10. 根据渲染树计算每个可见元素的布局（即尺寸和位置），然后将其绘制到屏幕上（在某些情况下，可以将内容提升到它们自己的层并进行合成，通过在 `GPU` 而不是 `CPU` 上绘制屏幕的一部分来提高性能，从而释放主线程。）
...
3. 将 `DOM` 树和 `CSSOM` 树 合成渲染树（Rendering Tree）
